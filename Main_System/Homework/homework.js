...
function addelements(){
  for (var a = 1; a <= localStorage["homeworkcounter"]; a = a + 1) {

    document.getElementById("table").insertAdjacentHTML('beforeEnd', "  <tr id=" + a + "><td></td><td><div style='visibility: hidden'>w</div></td><td>Subjects:</td><td><input type = 'text' size = '5' id = '" + "h" + "subject" + a + "'></td><td>Due Date:</td><td><input type = 'date' size ='4' id = '" + "h" + "date" + a + "'></td></tr><tr id=" + a + "n" + "><td><button id='delete' class='"+a+"'>Delete</button></td><td></td><td>Notes:</td><th colspan='4'><textarea rows = '4' cols = '30' id = '" + "h" + "notes" + a + "' style='width: 100%;'></textarea></th></tr>");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (localStorage["homeworkcounter"] === undefined) {
    localStorage["homeworkcounter"] = 0;
  }
  //load previous added homework
  addelements();

  if(parseInt(localStorage["homeworkcounter"] ) >= 1){
    document.getElementById("delete").addEventListener("click", function (callback) {
      alert("test");
      //console.log(element.classList.contains(class));
    });
  }
